	...
	for i := 0; i < len(fields); i++ {
		...
		case string:
			...
			case "SENTBEFORE":
				i++
				if date, ok := fields[i].(string); ok {
					c.SentBefore, _ = ParseDate(date)
				}
			case "SENTON":
				i++
				if date, ok := fields[i].(string); ok {
					c.SentOn, _ = ParseDate(date)
				}
			case "SENTSINCE":
				i++
				if date, ok := fields[i].(string); ok {
					c.SentSince, _ = ParseDate(date)
				}
			case "SINCE":
				i++
				if date, ok := fields[i].(string); ok {
					c.Since, _ = ParseDate(date)
				}
			case "SMALLER":
				i++
			...
			case "UNSEEN":
				c.Unseen = true
			default:
				return errors.New("Unknown search key")
			}
		}
	}
...
func (c *SearchCriteria) Format() (fields []interface{}) {
	...
	if c.Before != nil {
		fields = append(fields, "BEFORE", c.Before)
	}
	if c.Body != "" {
	...
	if c.SentBefore != nil {
		fields = append(fields, "SENTBEFORE", c.SentBefore)
	}
	if c.SentOn != nil {
		fields = append(fields, "SENTON", c.SentOn)
	}
	if c.SentSince != nil {
		fields = append(fields, "SENTSINCE", c.SentSince)
	}
	if c.Since != nil {
		fields = append(fields, "SINCE", c.Since)
	}
	if c.Smaller != 0 {
