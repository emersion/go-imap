...
// Write(). This ensures that compressed data is actually sent to the
// underlying connection after bufio.Writer.Flush() pushes its buffer.
// Without this, the flate.Writer may buffer data internally and the
// peer will hang waiting for data that never arrives on the wire.
